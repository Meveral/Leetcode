...
public:
	string shortestCompletingWord(string licensePlate, vector<string>& words)
	{
		...
		pair<string, pair<int,int>> ret{};
		int str_sz = licensePlate.size();
		vector<int> str_count(26, 0);
		for (auto&it : licensePlate)
		{
			if (it >= 'a'&&it <= 'z') ++str_count[it - 'a'];
			else if (it >= 'A'&&it <= 'Z') ++str_count[it - 'A'];
		}
		...
		for (int i = 0; i < words.size(); ++i)
		{
			...
			for (auto&it : words[i])
			{
				if (it >= 'a'&&it <= 'z') ++tmp_count[it - 'a'];
				else if (it >= 'A'&&it <= 'Z') ++tmp_count[it - 'A'];
			}

			int tmp_{};
			for (int j = 0; j < 26; ++j)
			{
				tmp_ += min(tmp_count[j], str_count[j]);
			}
			if (tmp_ > ret.second.first)
				ret = make_pair(words[i],make_pair(tmp_,words[i].size()));
			else if (tmp_ == ret.second.first)
			{
				if (words[i].size() < ret.second.second)
					ret = make_pair(words[i], make_pair(tmp_,words[i].size()));
			}
		}
